import asyncio
from playwright.async_api import async_playwright
import os

async def main():
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        page = await browser.new_page(viewport={"width": 1440, "height": 900})
        
        file_path = os.path.abspath('index.html')
        await page.goto(f'file:///{file_path}')
        await page.wait_for_timeout(1500)

        # Login and navigate explicitly to profile
        await page.evaluate("""async () => {
            document.getElementById('login-username-input').value = 'antonio';
            document.getElementById('login-password-input').value = '12345';
            await submitCustomLogin();
            if (typeof navigate === 'function') navigate('profile');
        }""")
        
        await page.wait_for_timeout(2000)

        # Take screenshot of profile section
        await page.screenshot(path="qa_profile_work_history_rendered.png")
        print("[SUCCESS] Screenshot captured successfully: qa_profile_work_history_rendered.png")
        await browser.close()

asyncio.run(main())
