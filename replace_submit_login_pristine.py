...
window.submitCustomLogin = submitCustomLogin;"""
    ...
    start_idx = content.find("function submitCustomLogin(targetUser) {")
    end_idx = content.find("window.submitCustomLogin = submitCustomLogin;", start_idx) + len("window.submitCustomLogin = submitCustomLogin;")

    if start_idx != -1 and end_idx != -1:
        content = content[:start_idx] + pristine_func + content[end_idx:]
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"Pristine submitCustomLogin applied to {filepath}")
    else:
        print(f"Error finding submitCustomLogin bounds in {filepath}")

replace_pristine(r"c:\Users\Antonio\.gemini\antigravity-ide\scratch\radar-comercial\index.html")
replace_pristine(r"c:\Users\Antonio\.gemini\antigravity-ide\scratch\radar-comercial\staging.html")
