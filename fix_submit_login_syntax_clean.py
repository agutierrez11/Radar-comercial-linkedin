...
def fix_submit_clean(filepath):
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()

    clean_submit_func = """function submitCustomLogin(targetUser) {
...
window.submitCustomLogin = submitCustomLogin;"""
    ...
    pattern = r'function submitCustomLogin\(targetUser\)\s*\{[\s\S]*?\n\}\nwindow\.submitCustomLogin\s*=\s*submitCustomLogin;'
    content = re.sub(pattern, clean_submit_func, content)

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)

fix_submit_clean(r"c:\Users\Antonio\.gemini\antigravity-ide\scratch\radar-comercial\index.html")
fix_submit_clean(r"c:\Users\Antonio\.gemini\antigravity-ide\scratch\radar-comercial\staging.html")
print("Clean submitCustomLogin function applied!")
